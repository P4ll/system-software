...
namespace Plugin
{
    public class SyntaxTree
    {
        ...
        private void state(Node par, int spaceCount)
        {
            if (_isEnd)
                return;
            if (_curTok.val2[0] == 'K')
            {
                if (spaceCount == 0 && par != beginNode)
                    par = beginNode;
                if (Lexer.words[_curTok.val1] == Lexer.Words.FOR)
                {
                    Node node = new Node(TypeNode.FOR1, spaceCount: spaceCount, position: _tokenInLine[_pos]);
                    par.childs.Add(node);
                    node.parent = par;
                    nextTok();
                    shiftSpace();
                    if (_curTok.val2[0] == 'I')
                    {
                        Node tempSet = new Node(TypeNode.SET, parent: node, position: _tokenInLine[_pos]);
                        node.childs.Add(tempSet);
                        Node tempVar = new Node(TypeNode.VAR, _curTok.val1, parent: tempSet, position: _tokenInLine[_pos]);
            ...
            else if (_curTok.val2[0] == 'I')
            {
                if (spaceCount == 0 && par != beginNode)
                    par = beginNode;
                Node node = new Node(TypeNode.SET, spaceCount: spaceCount, position: _tokenInLine[_pos]);
                par.childs.Add(node);
                node.parent = par;
                Node vInS = new Node(TypeNode.VAR, _curTok.val1, parent: node, position: _tokenInLine[_pos]);
            ...
            else if (equalLexSym(Lexer.Symbols.SPACE))
            {
                int sCountCur = 0;
                int sCountPrev = par.childs[par.childs.Count - 1].spaceCount;
                ...
                {
                    state(par.childs[par.childs.Count - 1], sCountCur);
                }
