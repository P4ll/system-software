...
namespace Plugin
{
    public static class CodeGenerator
    {
        public static string generateLWIQA(Node stNode)
        {
            ...
            addState(stNode.childs[0], sb, labelCount);
        ...
        private static void addState(Node node, StringBuilder sb, int labelCount)
        {
            ...
            Node parOfNode = node.parent;
            int i = 0;
            while (parOfNode.childs[i] != node) i++;
            if (i + 1 < parOfNode.childs.Count)
                addState(parOfNode.childs[i + 1], sb, labelCount);
        }
